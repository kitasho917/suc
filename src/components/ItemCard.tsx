...
type EditInput = Pick<PlannerItem, "title" | "memo" | "category" | "priority" | "isImportant" | "progress" | "timeBlock">;
type ItemCardProps = { item: PlannerItem; onToggle: (id: string) => void; onDelete: (id: string) => void; onEdit: (id: string, input: EditInput) => void };
...
export function ItemCard({ item, onToggle, onDelete, onEdit }: ItemCardProps) {
  ...
  const [timeBlock, setTimeBlock] = useState(item.timeBlock);

  const saveEdit = () => {
    const trimmed = title.trim();
    if (!trimmed) return;
    onEdit(item.id, { title: trimmed, memo: memo.trim() || undefined, category, priority, isImportant, progress, timeBlock });
    setIsEditing(false);
  };
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex-1">
          {isEditing ? <div className="space-y-2">
            ...
            <select value={timeBlock} onChange={(e) => setTimeBlock(e.target.value as PlannerItem["timeBlock"])} className="w-full rounded border border-gray-200 p-2 text-sm">{timeBlocks.map((block) => <option key={block} value={block}>時間帯: {timeBlockLabelMap[block]}</option>)}</select>
            ...
            <div className="flex gap-2"><button type="button" onClick={saveEdit} className="rounded bg-blue-600 px-3 py-1 text-xs text-white">保存</button><button type="button" onClick={() => setIsEditing(false)} className="rounded border border-gray-300 px-3 py-1 text-xs">キャンセル</button></div>
          </div> : <>
            ...
            <div className="mt-2 flex flex-wrap gap-2"><CategoryBadge category={item.category} /><PriorityBadge priority={item.priority} /><TimeBlockBadge timeBlock={item.timeBlock} />{item.isImportant ? <ImportantBadge /> : null}</div>
            <ProgressBar progress={item.progress} />
          </>}
        </div>
        <div className="flex flex-col gap-1"><button type="button" onClick={() => setIsEditing((prev) => !prev)} className="rounded-lg px-2 py-1 text-xs text-blue-600">編集</button><button type="button" onClick={() => onDelete(item.id)} className="rounded-lg px-2 py-1 text-xs text-red-600">削除</button></div>
      </div>
    </article>
  );
}
